                ...
                <table class="table table-bordered table-hover">
                    ...
                    <c:forEach items="${articleList}" var="article">
                            <tr>
                                ...
                                <td width="15%">
                                    <a href="${pageContext.request.contextPath}/article/deleteArticle?id=${article.articleId}<%--&pn=${articleMsgs.pageNum}&title=${articleSearch.title}&sendername=${articleSearch.sendername}--%>" role="button" class="btn btn-primary">屏蔽</a>
                                    <c:if test="${article.isTop==0}">
                                        <a href="${pageContext.request.contextPath}/article/changeStatus?id=${article.articleId}<%--&pn=${articleMsgs.pageNum}&title=${articleSearch.title}&sendername=${articleSearch.sendername}--%>" role="button" class="btn btn-danger" >置顶</a>
                                    </c:if>
                                    <c:if test="${article.isTop==1}">
                                        <a href="${pageContext.request.contextPath}/article/changeStatus?id=${article.articleId}<%--&pn=${articleMsgs.pageNum}&title=${articleSearch.title}&sendername=${articleSearch.sendername}--%>" role="button" class="btn btn-info" >取消</a>
                                    </c:if>
                                </td>
                            </tr>
                        </c:forEach>
